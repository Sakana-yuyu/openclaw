...
export function renderNostrCard(params: {
      ...
      <div class="account-card">
        ...
        <div class="status-list account-card-status">
          <div>
            ...
            <span>${account.running ? t("cron.summary.yes") : t("cron.summary.no")}</span>
          </div>
          <div>
          ...
          <div>
            <span class="label">${t("channelsPage.nostr.publicKey")}</span>
            <span class="monospace" title="${publicKey ?? ""}">${truncatePubkey(publicKey)}</span>
          </div>
          <div>
            <span class="label">${t("channelsPage.fields.lastInbound")}</span>
    ...
    return html`
      <div style="margin-top: 16px; padding: 12px; background: var(--bg-secondary); border-radius: 8px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
          <div style="font-weight: 500;">${t("channelsPage.nostr.profile")}</div>
          ${
            summaryConfigured
              ? html`
                <button
                ...
                >
                  ${t("channelsPage.nostr.editProfile")}
                </button>
              `
              : nothing
          }
          ...
          hasAnyProfileData
            ? html`
              <div class="status-list">
                  ...
                  picture
                    ? html`
                      <div style="margin-bottom: 8px;">
                        <img
                          src=${picture}
                          alt=${t("channelsPage.nostr.profilePictureAlt")}
                ...
                ${name ? html`<div><span class="label">${t("channelsPage.nostr.name")}</span><span>${name}</span></div>` : nothing}
                ${
                  displayName
                    ? html`<div><span class="label">${t("channelsPage.nostr.displayName")}</span><span>${displayName}</span></div>`
                    : nothing
                }
                ${
                  about
                    ? html`<div><span class="label">${t("channelsPage.nostr.about")}</span><span style="max-width: 300px; overflow: hidden; text-overflow: ellipsis;">${about}</span></div>`
                    : nothing
                }
                ${
                  nip05
                    ? html`<div><span class="label">${t("channelsPage.nostr.nip05")}</span><span>${nip05}</span></div>`
                    : nothing
                }
              </div>
            `
            : html`
                <div style="color: var(--text-muted); font-size: 13px">
                  ${t("channelsPage.nostr.noProfileSet")}
                </div>
              `
        }
      </div>
  ...
  return html`
    <div class="card">
      <div class="card-title">${t("channelsPage.nostr.title")}</div>
      <div class="card-sub">${t("channelsPage.nostr.subtitle")}</div>
      ${accountCountLabel}
        ...
        hasMultipleAccounts
          ...
          : html`
            <div class="status-list" style="margin-top: 16px;">
              <div>
                <span class="label">${t("channelsPage.fields.configured")}</span>
                <span>${summaryConfigured ? t("cron.summary.yes") : t("cron.summary.no")}</span>
              </div>
              <div>
                <span class="label">${t("channelsPage.fields.running")}</span>
                <span>${summaryRunning ? t("cron.summary.yes") : t("cron.summary.no")}</span>
              </div>
              <div>
                <span class="label">${t("channelsPage.nostr.publicKey")}</span>
