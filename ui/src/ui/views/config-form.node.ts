...
export function renderNode(params: {
  ...
  if (unsupported.has(key)) {
    return html`<div class="cfg-field cfg-field--error">
      ...
      <div class="cfg-field__error">${t("configPage.node.unsupportedSchemaNode")}</div>
    </div>`;
  }
  if (
    criteria &&
  ...
  return html`
    <div class="cfg-field cfg-field--error">
      <div class="cfg-field__label">${label}</div>
      <div class="cfg-field__error">${t("configPage.node.unsupportedType", { type: String(type) })}</div>
    </div>
  `;
}
  ...
  const placeholder =
    ...
    (isSensitive
      ...
      : schema.default !== undefined
        ? t("configPage.node.defaultValue", { value: String(schema.default) })
        : "");
      ...
      <div class="cfg-input-wrap">
        ...
        ${
          ...
          <button
            ...
            title=${t("configPage.node.resetToDefault")}
            ?disabled=${disabled}
...
function renderSelect(params: {
  ...
  return html`
    <div class="cfg-field">
      ...
      >
        <option value=${unset}>${t("configPage.node.select")}</option>
        ${options.map(
          (opt, idx) => html`
...
function renderArray(params: {
  ...
  if (!itemsSchema) {
    return html`
      <div class="cfg-field cfg-field--error">
        <div class="cfg-field__label">${label}</div>
        <div class="cfg-field__error">${t("configPage.node.unsupportedArraySchema")}</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="cfg-array">
      <div class="cfg-array__header">
        ...
        <span class="cfg-array__count">${t("configPage.node.itemCount", { count: String(arr.length) })}</span>
        <button
          type="button"
      ...
      ${
        arr.length === 0
          ? html`
              <div class="cfg-array__empty">${t("configPage.node.noItems")}</div>
            `
          : html`
            ...
            <div class="cfg-array__item">
              <div class="cfg-array__item-header">
                ...
                <button
                  ...
                  title=${t("configPage.node.removeItem")}
                  ?disabled=${disabled}
                  @click=${() => {
                    const next = [...arr];
                    next.splice(idx, 1);
  ...
  return html`
    <div class="cfg-map">
      <div class="cfg-map__header">
        <span class="cfg-map__label">${t("configPage.node.customEntries")}</span>
        <button
          type="button"
          class="cfg-map__add"
          ?disabled=${disabled}
      ...
      ${
        visibleEntries.length === 0
          ? html`
              <div class="cfg-map__empty">${t("configPage.node.noCustomEntries")}</div>
            `
          : html`
                ...
                <div class="cfg-map__item-header">
                  <div class="cfg-map__item-key">
                    <input
                      ...
                      placeholder=${t("configPage.node.keyPlaceholder")}
                      .value=${key}
                      ?disabled=${disabled}
                      @change=${(e: Event) => {
                  ...
                  <button
                    ...
                    title=${t("configPage.node.removeEntry")}
                    ?disabled=${disabled}
                    @click=${() => {
                      const next = { ...value };
                      delete next[key];
                    ...
                    anySchema
                      ? html`
                        <textarea
                          ...
                          placeholder=${t("configPage.node.jsonValue")}
                          rows="2"
                          .value=${fallback}
                          ?disabled=${disabled}
                          @change=${(e: Event) => {
