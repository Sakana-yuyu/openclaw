...
export function renderChannelConfigForm(props: ChannelConfigFormProps) {
  ...
  if (!normalized) {
    return html`
      <div class="callout danger">${t("channelsPage.config.schemaUnavailable")}</div>
    `;
  }
  const node = resolveSchemaNode(normalized, ["channels", props.channelId]);
  if (!node) {
    return html`
      <div class="callout danger">${t("channelsPage.config.channelSchemaUnavailable")}</div>
    `;
  }
    ...
    <div style="margin-top: 16px;">
      ${
        props.configSchemaLoading
          ? html`
              <div class="muted">${t("channelsPage.config.loadingConfigSchema")}</div>
            `
      ...
      <div class="row" style="margin-top: 12px;">
        ...
        >
          ${props.configSaving ? t("agentsPage.common.saving") : t("agentsPage.common.save")}
        </button>
        <button
          class="btn"
          ?disabled=${disabled}
