...
export function renderUsage(props: UsageProps) {
        ...
        <div class="row" style="justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 12px;">
          <div style="flex: 1; min-width: 250px;">
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 2px;">
              <div class="card-title" style="margin: 0;">${t("usagePage.header.title")}</div>
              <span style="
                display: inline-flex;
                align-items: center;
                gap: 6px;
          ...
          <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 8px;">
            <div style="display: flex; gap: 8px; align-items: center;">
              ...
              <span style="color: var(--muted);">${t("usagePage.header.to")}</span>
  ...
  const hasMissingCost =
    ...
    (displayTotals
      ...
      : false);
  const datePresets = [
    { label: t("usagePage.header.today"), days: 1 },
    { label: t("usagePage.header.sevenDays"), days: 7 },
    { label: t("usagePage.header.thirtyDays"), days: 30 },
  ];
  ...
  return html`
    ...
    <section class="usage-page-header">
      <div class="usage-page-title">${t("usagePage.header.title")}</div>
      <div class="usage-page-subtitle">${t("usagePage.header.subtitle")}</div>
    </section>

    <section class="card usage-header ${props.headerPinned ? "pinned" : ""}">
      <div class="usage-header-row">
        <div class="usage-header-title">
          <div class="card-title" style="margin: 0;">${t("usagePage.header.filters")}</div>
          ${
            props.loading
              ? html`
                  <span class="usage-refresh-indicator">${t("usagePage.header.loading")}</span>
                `
              : nothing
          }
          ${
            isEmpty
              ? html`
                  <span class="usage-query-hint">${t("usagePage.header.noData")}</span>
                `
              : nothing
          }
        </div>
        <div class="usage-header-metrics">
            ...
            displayTotals
              ? html`
                <span class="usage-metric-badge">
                  <strong>${formatTokens(displayTotals.totalTokens)}</strong> ${t("usagePage.header.tokens").toLowerCase()}
                </span>
                <span class="usage-metric-badge">
                  <strong>${formatCost(displayTotals.totalCost)}</strong> ${t("usagePage.header.cost").toLowerCase()}
                </span>
                <span class="usage-metric-badge">
                  <strong>${displaySessionCount}</strong>
                  ${t("usagePage.header.sessions").toLowerCase()}
                </span>
              `
              : nothing
          }
          <button
            ...
            title=${props.headerPinned ? t("usagePage.header.unpin") : t("usagePage.header.pin")}
            @click=${props.onToggleHeaderPinned}
          >
            ${props.headerPinned ? t("usagePage.header.pinned") : t("usagePage.header.pin")}
          </button>
          <details
          ...
          >
            <summary class="usage-export-button">${t("usagePage.header.export")} ▾</summary>
            <div class="usage-export-popover">
              <div class="usage-export-list">
                ...
                >
                  ${t("usagePage.header.sessionsCsv")}
                </button>
                <button
                  class="usage-export-item"
                  @click=${() =>
      ...
      <div class="usage-header-row">
        <div class="usage-controls">
          ...
          <input
            ...
            title="${t("usagePage.header.startDate")}"
          ...
          <input
            type="date"
            .value=${props.endDate}
            title="${t("usagePage.header.endDate")}"
            @change=${(e: Event) => props.onEndDateChange((e.target as HTMLInputElement).value)}
          />
          <select
            title="${t("usagePage.header.timezone")}"
            .value=${props.timeZone}
            @change=${(e: Event) =>
              props.onTimeZoneChange((e.target as HTMLSelectElement).value as "local" | "utc")}
          >
            <option value="local">${t("usagePage.header.local")}</option>
            <option value="utc">${t("usagePage.header.utc")}</option>
          </select>
          <div class="chart-toggle">
            ...
            >
              ${t("usagePage.header.tokens")}
            </button>
            <button
              class="toggle-btn ${!isTokenMode ? "active" : ""}"
              @click=${() => props.onChartModeChange("cost")}
            >
              ${t("usagePage.header.cost")}
            </button>
          </div>
          <button
      ...
      <div style="margin-top: 12px;">
          ...
          <input
            ...
            placeholder="${t("usagePage.header.filterPlaceholder")}"
          ...
          <div class="usage-query-actions">
            ...
            >
              ${t("usagePage.header.filterClientSide")}
            </button>
            ${
              hasDraftQuery || hasQuery
                ? html`<button class="btn btn-sm usage-action-btn usage-secondary-btn" @click=${props.onClearQuery}>${t("usagePage.header.clear")}</button>`
                : nothing
            }
            <span class="usage-query-hint">
              ${
                hasQuery
                  ? t("usagePage.header.sessionsMatch").replace("{count}", `${filteredSessions.length}`).replace("{total}", `${totalSessions}`)
                  : t("usagePage.header.sessionsInRange").replace("{total}", `${totalSessions}`)
              }
            </span>
          </div>
        </div>
        <div class="usage-filter-row">
          ${renderFilterSelect("agent", t("usagePage.header.agent"), agentOptions)}
          ${renderFilterSelect("channel", t("usagePage.header.channel"), channelOptions)}
          ${renderFilterSelect("provider", t("usagePage.header.provider"), providerOptions)}
          ${renderFilterSelect("model", t("usagePage.header.model"), modelOptions)}
          ${renderFilterSelect("tool", t("usagePage.header.tool"), toolOptions)}
          <span class="usage-query-hint">
            ${t("usagePage.header.filterTip")}
          </span>
        </div>
        ${
          queryTerms.length > 0
        ...
        props.sessionsLimitReached
          ? html`
              <div class="callout warning" style="margin-top: 12px">
                ${t("usagePage.header.limitWarning")}
              </div>
            `
          : nothing
      }
