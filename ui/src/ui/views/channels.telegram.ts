...
export function renderTelegramCard(params: {
  ...
  return html`
    <div class="card">
      <div class="card-title">${t("channelsPage.telegram.title")}</div>
      <div class="card-sub">${t("channelsPage.telegram.subtitle")}</div>
      ${accountCountLabel}
        ...
        hasMultipleAccounts
          ...
          : html`
            <div class="status-list" style="margin-top: 16px;">
              <div>
                <span class="label">${t("channelsPage.fields.configured")}</span>
                <span>${telegram?.configured ? t("cron.summary.yes") : t("cron.summary.no")}</span>
              </div>
              <div>
                <span class="label">${t("channelsPage.fields.running")}</span>
                <span>${telegram?.running ? t("cron.summary.yes") : t("cron.summary.no")}</span>
              </div>
              <div>
                <span class="label">${t("channelsPage.telegram.mode")}</span>
      ...
      ${
        telegram?.probe
          ? html`<div class="callout" style="margin-top: 12px;">
            ${t("channelsPage.common.probe")} ${telegram.probe.ok ? t("channelsPage.common.probeOk") : t("channelsPage.common.probeFailed")} ·
