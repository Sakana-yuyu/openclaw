  ...
  if (status.active) {
    return html`
      <div class="compaction-indicator compaction-indicator--active" role="status" aria-live="polite">
        ${icons.loader} ${t("chat.compactionActive")}
      </div>
    `;
  }
    ...
    if (elapsed < COMPACTION_TOAST_DURATION_MS) {
      return html`
        <div class="compaction-indicator compaction-indicator--complete" role="status" aria-live="polite">
          ${icons.check} ${t("chat.compactionComplete")}
        </div>
      `;
    }
  }
...
function renderFallbackIndicator(status: FallbackIndicatorStatus | null | undefined) {
  ...
  const details = [
    ...
    phase === "cleared"
      ? t("chat.fallback.active", { value: status.selected })
  ...
  const message = phase === "cleared"
    ...
    : t("chat.fallback.activeNow", { value: status.active });
  const className =
    phase === "cleared"
        ...
        (att) => html`
          <div class="chat-attachment">
            ...
            <button
              ...
              type="button"
              aria-label=${t("chat.attachment.removeAria")}
              @click=${() => {
...
export function renderChat(props: ChatProps) {
  ...
  const composePlaceholder = props.connected
    ? hasAttachments
      ? t("chat.placeholders.withImages")
      : t("chat.placeholders.default")
    : t("chat.placeholders.disconnected");
                  ...
                  (item) => html`
                    <div class="chat-queue__item">
                      <div class="chat-queue__text">
                        ${
                          item.text ||
                          (item.attachments?.length ? t("chat.queue.imageItem", { count: String(item.attachments.length) }) : "")
                        }
                      </div>
                      <button
      ...
      <div class="chat-compose">
        ...
        <div class="chat-compose__row">
          ...
          <div class="chat-compose__actions">
            ...
            >
              ${canAbort ? t("chat.actions.stop") : t("chat.actions.newSession")}
            </button>
            <button
              class="btn primary"
              ?disabled=${!props.connected}
              @click=${props.onSend}
            >
              ${isBusy ? t("chat.actions.queue") : t("chat.actions.send")}<kbd class="btn-kbd">↵</kbd>
            </button>
          </div>
        </div>
      </div>
...
function buildChatItems(props: ChatProps): Array<ChatItem | MessageGroup> {
  ...
  // This ensures correct visual ordering: text 鈫?tool 鈫?text 鈫?tool 鈫?...
