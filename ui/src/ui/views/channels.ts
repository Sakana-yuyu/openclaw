...
${props.snapshot ? JSON.stringify(props.snapshot, null, 2) : t("channelsPage.health.noSnapshotYet")}
      </pre>
    </section>
  `;
}
...
function renderGenericChannelCard(
  ...
  return html`
    <div class="card">
      ...
      <div class="card-sub">${t("channelsPage.channel.subtitle")}</div>
      ${accountCountLabel}
...
function formatBool(value: boolean | undefined): string {
  ...
  return value ? t("cron.summary.yes") : t("cron.summary.no");
}
...
function deriveRunningStatus(account: ChannelAccountSnapshot): string {
  if (account.running) {
    return t("cron.summary.yes");
  }
  // If we have recent inbound activity, the channel is effectively running
  if (hasRecentActivity(account)) {
    return t("channelsPage.common.active");
  }
  return t("cron.summary.no");
}
