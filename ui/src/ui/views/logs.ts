...
export function renderLogs(props: LogsProps) {
  ...
  const exportLabel = needle || levelFiltered
    ? t("logsPage.exportLabel.filtered")
    : t("logsPage.exportLabel.visible");
  ...
  return html`
    <section class="card">
      ...
      ${
        props.file
          ? html`<div class="muted" style="margin-top: 10px;">${t("logsPage.fileLabel")}: ${props.file}</div>`
          : nothing
      }
      ${
        props.truncated
      ...
      <div class="log-stream" style="margin-top: 12px;" @scroll=${props.onScroll}>
        ${
          filtered.length === 0
            ? html`
                <div class="muted" style="padding: 12px">${t("logsPage.noEntries")}</div>
              `
            : filtered.map(
                (entry) => html`
                <div class="log-row">
